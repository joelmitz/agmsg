...
```http
Agmsg-Client-Version: <client-version>
```

on any request, including health. This is metadata, not authentication or
authorization; a server MUST NOT reject a request solely for carrying an
unexpected, missing, or malformed value in this header, and MUST ignore it if
the value is not understood. A request without this header comes from a
client older than 1.3.1, the release that introduced it; a client that sends
it but could not determine its own version sends the literal value `unknown`
rather than omitting the header or sending an empty one, so a server can tell
"announced, but unreadable" apart from "never announces a version at all."
The value, when present, is at most 64 printable ASCII characters.
