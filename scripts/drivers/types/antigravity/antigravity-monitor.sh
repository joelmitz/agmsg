...
# unreadable lock.
command -v node >/dev/null || { echo 'Antigravity monitor requires node (not found on PATH)' >&2; exit 1; }
if [ "$(uname -s)" = Darwin ]; then
  command -v python3 >/dev/null || { echo 'Antigravity monitor requires python3 for advisory locks on macOS' >&2; exit 1; }
else
  command -v flock >/dev/null || { echo 'Antigravity monitor requires flock (not found on PATH)' >&2; exit 1; }
fi
