...
usage() {
  cat <<'EOF'
Usage: codex-diag.sh <project> <team> <agent> [--self-test|--confirm NONCE MESSAGE_ID|--status DIAGNOSIS_ID]
...
PROJECT="${1:?Usage: codex-diag.sh <project> <team> <agent> [--self-test|--confirm NONCE MESSAGE_ID|--status DIAGNOSIS_ID]}"
TEAM="${2:?Missing team}"
...
case "${1:-}" in
  "") ;;
  --self-test) [ "$#" -eq 1 ] || { echo "codex-diag.sh: --self-test takes no value" >&2; exit 2; }; ACTION="self-test" ;;
  --confirm) [ "$#" -eq 3 ] || { echo "codex-diag.sh: --confirm requires NONCE MESSAGE_ID" >&2; exit 2; }; ACTION="confirm"; ACTION_VALUE="$2"; CONFIRM_MESSAGE_ID="$3" ;;
  --status) [ "$#" -eq 2 ] || { echo "codex-diag.sh: --status requires DIAGNOSIS_ID" >&2; exit 2; }; ACTION="status"; ACTION_VALUE="$2" ;;
  *) echo "codex-diag.sh: unknown option: $1" >&2; exit 2 ;;
esac
...
is_windows_git_bash() {
  case "${MSYSTEM:-}" in MINGW*|MSYS*|CLANGARM*) return 0 ;; *) return 1 ;; esac
}
probe_ss_pids() {
...
TUI_PIDS=""
process_reason="no-matching-tui"
probe_ok=0
: >"$DIAG_TMP/pids"
if is_windows_git_bash; then
  # WinPID is authoritative. Empty ss is absent auxiliary evidence; MSYS PIDs are not mixed in.
