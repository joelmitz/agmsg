...
if [ "$UPDATE_ONLY" = true ]; then
  ...
  echo ""
  echo "    Codex monitor: restart Codex through the configured shell/PATH shim,"
  echo "      send one first turn, then run:"
  echo "      delivery.sh status codex <project>"
  echo ""
  echo "    Antigravity TUI monitor: restart it through the installed launcher,"
  echo "      then check the same role without reading its inbox directly:"
  echo "      agy-tui --team <team> --name <role>"
  echo "      agy-tui status --project <project> --team <team> --name <role>"
  echo ""
...
echo "    3. For real-time delivery, start the runtime-specific monitor:"
echo "       Codex: restart through the shell/PATH shim printed by mode monitor,"
echo "              then send one first turn and check delivery.sh status codex <project>"
echo "       Antigravity TUI: run agy-tui --team <team> --name <role>,"
echo "                        then check with agy-tui status (not a direct inbox read)"
echo ""
